...
public class RegionModelRefactoring extends Refactoring {
	...
	public RefactoringStatus checkFinalConditions(final IProgressMonitor pm)
		...
		for (final PrepRunDescription desc : info.getSelectedRuns()) {
			final DBConnection conn = desc.getDescription().getDB();
			try {
				conn.bootAndCheckSchema();
				mergeRegions(targetRegions, conn.withReadOnly(RecommendRegions
						.lockIsThisRegions()));
				mergeRegions(targetClassRegions, conn
						.withReadOnly(RecommendRegions.lockIsClassRegions()));
		...
		for (final IPackageFragment fragment : info.getSelectedProject()
			...
			for (final ICompilationUnit cu : fragment.getCompilationUnits()) {
				...
				if (!model.isEmpty()) {
					...
					rewrite.writeAnnotations(model.generateAnnotations(typeMap,
							ns), g);
	...
	// @Override
	// public Change createChange(final IProgressMonitor pm) throws
	// CoreException,
	// OperationCanceledException {
	// final CompositeChange root = new CompositeChange(String.format(
	// "Changes to %s", info.getSelectedProject().getProject()
	// .getName()));
	// final RegionRewriter rewriter = new RegionRewriter(info
	// .getSelectedProject());
	// final TextEditGroup g = new TextEditGroup(
	// "Adding @RegionLock annotations");
	// for (final IPackageFragment fragment : info.getSelectedProject()
	...
	// final Map<String, RecommendedRegion> targetClassMap = targetClassRegions
	// .get(packageName);
	// final Map<String, List<RecommendedRegion>> targetFieldMap =
	// targetFieldRegions
	// .get(packageName);
	// for (final ICompilationUnit cu : fragment.getCompilationUnits()) {
	// rewriter.setCompilationUnit(cu);
	// final RegionModel model = new RegionModel();
	// for (final IType type : cu.getTypes()) {
	// final String typeName = type.getElementName();
	// model.mergeRegion(targetMap == null ? null : targetMap
	...
	// if (!model.isEmpty()) {
	//
	// rewriter.rewriteRegionModel(g, model);
	// final TextEdit edit = rewriter.getTextEdit();
	// final IFile file = (IFile) cu.getResource();
	// final TextFileChange change = new TextFileChange(file
	// .getName(), file);
	// change.setEdit(edit);
