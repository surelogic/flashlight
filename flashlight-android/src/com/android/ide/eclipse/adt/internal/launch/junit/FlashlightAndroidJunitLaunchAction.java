...
/**
 * A launch action that executes a instrumentation test run on an Android
 * device.
 */
class FlashlightAndroidJUnitLaunchAction implements IAndroidLaunchAction {
     ...
     *
     * @param launchInfo
     *            the {@link AndroidJUnitLaunchInfo} for the JUnit run
     */
    public FlashlightAndroidJUnitLaunchAction(AndroidJUnitLaunchInfo launchInfo) {
        mLaunchInfo = launchInfo;
    }

    /**
     * Launch a instrumentation test run on given Android devices. Reuses JDT
     * JUnit launch delegate so results can be communicated back to JDT JUnit
     * UI.
     * <p/>
     ...
     */
    @Override
    public boolean doLaunchAction(DelayedLaunchInfo info,
            Collection<IDevice> devices) {
        String msg = String.format(
                LaunchMessages.AndroidJUnitLaunchAction_LaunchInstr_2s,
        ...
        try {
            ...
            JUnitLaunchDelegate junitDelegate = new JUnitLaunchDelegate(
                    mLaunchInfo);
            final String mode = info.isDebugMode() ? ILaunchManager.DEBUG_MODE
                    : ILaunchManager.RUN_MODE;

            junitDelegate.launch(info.getLaunch().getLaunchConfiguration(),
                    mode, info.getLaunch(), info.getMonitor());
    ...
    public String getLaunchDescription() {
        return String.format(
                LaunchMessages.AndroidJUnitLaunchAction_LaunchDesc_s,
     ...
     */
    private static class JUnitLaunchDelegate extends
    JUnitLaunchConfigurationDelegate {
         ...
         *
         * @see
         * org.eclipse.jdt.junit.launcher.JUnitLaunchConfigurationDelegate#launch
        ...
        /**
         * Overrides parent to return a VM Runner implementation which launches
         * a thread, rather than a separate VM process
         */
        @Override
    ...
    /**
     * Provides a VM runner implementation which starts a inline implementation
     * of a launch process
     */
    private static class VMTestRunner implements IVMRunner {
        ...
        public void run(final VMRunnerConfiguration config, ILaunch launch,
            ...
            TestRunnerProcess runnerProcess = new TestRunnerProcess(config,
                    mJUnitInfo);
    ...
    private static class TestRunnerProcess implements IProcess {
        ...
        TestRunnerProcess(VMRunnerConfiguration runConfig,
         ...
         *
         * @see
         * org.eclipse.debug.core.model.IProcess#getAttribute(java.lang.String)
         */
        @Override
         ...
         *
         * @see
         * org.eclipse.debug.core.model.IProcess#setAttribute(java.lang.String,
         * java.lang.String)
         */
        @Override
        ...
        /**
         * Launches a test runner that will communicate results back to JDT
         * JUnit UI.
         * <p/>
        ...
        public void run() {
            if (Display.getCurrent() != null) {
                AdtPlugin.log(IStatus.ERROR,
                        "Adt test runner executed on UI thread");
                AdtPlugin
                .printErrorToConsole(mJUnitInfo.getProject(),
