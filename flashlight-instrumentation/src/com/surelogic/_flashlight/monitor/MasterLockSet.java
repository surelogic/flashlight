...
public class MasterLockSet {
  ...
  /**
   * drain grabs the lock set results from the given LockSets object and merges
   * them with its results. The given LockSets object is reset to a pristine
   * state, although the locks currently held is kept.
   * 
   * @param other
   */
  synchronized void drain(final ThreadLocks other) {
    ...
    for (final Entry<Long, Map<Long, Set<Long>>> e : otherLockSets.entrySet()) {
      final long receiverId = e.getKey();
  ...
  static class Graph {
    ...
    Set<Long> cycles() {
      ...
      for (final Entry<Long, Set<Long>> vertex : reachable.entrySet()) {
        final long from = vertex.getKey();
        for (final long to : vertex.getValue()) {
          if (to != from && reachable.get(to).contains(from)) {
            cycleLocks.add(from);
            cycleLocks.add(to);
          }
        }
   ...
   * results. This method should be called when an object is garbage collected.
   * 
   * @param receiverId
  ...
  /**
   * Return a {@link LockSetInfo} showing the lock sets for all fields that are
   * actively shared in the program.
   * 
   * @return
   */
  LockSetInfo getLockSetInfo() {
    final Map<Long, Set<Long>> statics = new HashMap<Long, Set<Long>>(staticLockSets.size());
    ...
    // what we want
    final Map<Long, Map<Long, Set<Long>>> instances = new HashMap<Long, Map<Long, Set<Long>>>(lockSets.size());
    ...
    return new LockSetInfo(defs, statics, new HashSet<Long>(lockSetFields), new HashSet<Long>(noLockSetFields), instances);
  }
