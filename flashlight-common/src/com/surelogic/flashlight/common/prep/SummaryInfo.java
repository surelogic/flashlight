...
public class SummaryInfo {
  ...
  public SummaryInfo(final LimitedResult<Lock> locks, final LimitedResult<Thread> threads,
      final List<LockSetEvidence> emptyLockSetFields, final List<BadPublishEvidence> badPublishes,
      final LimitedResult<DeadlockEvidence> deadlocks, final String objectCount, final String classCount,
  ...
  public static class SummaryQuery implements DBQuery<SummaryInfo> {
    ...
    public SummaryInfo perform(final Query q) {
      ...
      List<BadPublishEvidence> badPublishes = q.prepared("SummaryInfo.badPublishes", new BadPublishEvidenceHandler(q)).call();
      LimitedResult<DeadlockEvidence> deadlocks = q.prepared("Deadlock.lockCycles", new DeadlockEvidenceHandler(q)).call();
      String classCount = q.prepared("SummaryInfo.classCount", new StringResultHandler()).call();
      String objectCount = q.prepared("SummaryInfo.objectCount", new StringResultHandler()).call();
    ...
    void process(final Query q, final CoverageSite site, final Map<Long, Set<Long>> map) {
  ...
  public static class LockSetEvidence implements Comparable<LockSetEvidence>, Loc {
    private final Field field;
    private final LimitedResult<LockSetLock> likelyLocks;

    public LockSetEvidence(final Field field, final LimitedResult<LockSetLock> likelyLocks) {
      this.field = field;
  ...
  public static class LockSetLock {
    ...
    public String toString() {
      return "LockSetLock [name=" + name + ", id=" + id + ", heldPercentage=" + heldPercentage + ", acquisitions=" + heldAt
          + ", notHeldAt=" + notHeldAt + "]";
    }
  ...
  /**
   * Represents a site that has been accessed while a lock was held. It behaves
   * the same as the field access site would, but has a getter to see where the
   * lock was acquired.
   * 
   * @author nathan
   * 
  ...
  private static class LockSetEvidenceHandler implements RowHandler<LockSetLock> {
  ...
  public static class DeadlockTrace implements Comparable<DeadlockTrace> {
    ...
    public DeadlockTrace(final Edge edge, final List<Trace> trace, final List<Trace> heldTrace, final List<LockTrace> lockTrace) {
      this.edge = edge;
      this.trace = trace;
  ...
  public static class LockTrace {
    ...
    public LockTrace(final long id, final String lock, final String pakkage, final String clazz, final int line) {
      this.id = id;
      this.lock = lock;
  ...
  static class LockTraceHandler implements RowHandler<LockTrace> {
    ...
    public LockTrace handle(final Row r) {
      return new LockTrace(r.nextLong(), r.nextString(), r.nextString(), r.nextString(), r.nextInt());
    }
  }
  ...
  static class ContentionSitesHandler implements ResultHandler<List<ContentionSite>> {
  ...
  private static class BadPublishEvidenceHandler implements RowHandler<BadPublishEvidence> {
    static final int BAD_PUBLISH_ACCESS_LIMIT = 50;
    final Query q;
    ...
    public BadPublishEvidence handle(final Row r) {
      ...
      q.prepared("SummaryInfo.underConstructionAccesses", LimitRowHandler.from(new RowHandler<BadPublishAccess>() {
        @Override
        public BadPublishAccess handle(final Row r) {
          ...
          List<Trace> trace = Trace.stackTrace(r.nextLong()).perform(q);

          return new BadPublishAccess(thread, isRead, time, trace);
        }
      }, BAD_PUBLISH_ACCESS_LIMIT)).call(f.getId(), f.getReceiver()));
      return e;
    }
  }
  ...
  public static class BadPublishEvidence implements Loc {
    ...
    public BadPublishEvidence(final Field f, final LimitedResult<BadPublishAccess> accesses) {
      this.f = f;
  ...
  public static class BadPublishAccess {
    ...
    public BadPublishAccess(final String thread, final boolean isRead, final Timestamp time, final List<Trace> trace) {
      this.thread = thread;
      this.isRead = isRead;
      this.time = time;
      this.trace = trace;
  ...
  public static class Field implements Comparable<Field>, Loc {
    ...
    public Field(final String pakkage, final String clazz, final String name, final long id, final boolean isStatic,
        final Long receiver) {
      this.pakkage = pakkage;
      this.clazz = clazz;
      this.name = name;
    ...
    @Override
    public String toString() {
      return "Field [pakkage=" + pakkage + ", clazz=" + clazz + ", name=" + name + ", id=" + id + ", isStatic=" + isStatic + "]";
    }

  }

  static class FieldHandler implements RowHandler<Field> {
  ...
  public static class Thread implements Comparable<Thread> {
    ...
    public Thread(final String id, final String name, final Date start, final Date stop, final long blockTime) {
      this.id = id;
      this.name = name;
      this.start = start;
      this.stop = stop;
  ...
  static class ThreadContentionHandler implements RowHandler<Thread> {

    @Override
    public Thread handle(final Row r) {
      return new Thread(r.nextString(), r.nextString(), r.nextTimestamp(), r.nextTimestamp(), r.nextLong());
    }
  ...
  public static class Lock {
    ...
    public Lock(final String name, final int acquired, final long blockTime, final long averageBlock, final long id) {
  ...
  private static class LockContentionHandler implements RowHandler<Lock> {
    ...
    public LockContentionHandler(final Query q) {
      prepared = q.prepared("SummaryInfo.lockContentionSites", new ContentionSitesHandler());
    }

    @Override
    public Lock handle(final Row r) {
      Lock l = new Lock(r.nextString(), r.nextInt(), r.nextLong(), r.nextLong(), r.nextLong());
  ...
  public static class Edge implements Comparable<Edge> {
    ...
    Edge(final String held, final String heldId, final String acquired, final String acquiredId, final int count,
        final Timestamp first, final Timestamp last, final List<String> threads) {
      this.held = held;
      this.heldId = heldId;
    ...
    public int hashCode() {
      final int prime = 31;
      int result = 1;
      result = prime * result + (acquiredId == null ? 0 : acquiredId.hashCode());
    ...
    @Override
    public String toString() {
      return "Edge [held=" + held + ", acquired=" + acquired + ", count=" + count + ", first=" + first + ", last=" + last + "]";
    }
   ...
   * 
   */
  private static class DeadlockEvidenceHandler implements ResultHandler<LimitedResult<DeadlockEvidence>> {
  ...
  private static class DeadlockTraceHandler implements ResultHandler<DeadlockTrace> {
    private final Query q;
    private final Edge edge;
    ...
    public DeadlockTrace handle(final Result result) {
      for (Row r : result) {
        ...
        List<Trace> heldTrace = Trace.stackTrace(heldTraceId).perform(q);
        List<LockTrace> lockTrace = q.prepared("Deadlock.lockEdgeLockTrace", new LockTraceHandler()).call(lockEventId);
  ...
  static class SiteHandler implements RowHandler<Site> {
    @Override
    public Site handle(final Row r) {
      return new Site(r.nextString(), r.nextString(), r.nextString(), r.nextInt(), r.nextString(), r.nextString(), r.nextString());
    }
  }

  static class LockSetSiteHandler implements RowHandler<LockSetSite> {
  ...
  public static class Site implements Comparable<Site>, Loc {
    ...
    public Site(final String pakkage, final String clazz, final String location, final int line, final String file,
    ...
    @Override
    public String toString() {
      return "Site [pakkage=" + pakkage + ", clazz=" + clazz + ", name=" + location + "]";
    }
  ...
  public static class CoverageSite implements Comparable<CoverageSite> {
    ...
    public CoverageSite() {
      this(new Site("*root*", "*root*", "*root*", 0, "*root*", null, null));
    }
    ...
    void addTraceHelper(final Trace t, final Iterator<Trace> rest, final Set<Long> threads) {

      Site s = new Site(t.getPackage(), t.getClazz(), t.getLoc(), t.getLine(), t.getFile(), t.getMethodCallClass(),
          t.getMethodCallName());
      ...
      if (rest.hasNext()) {
        ...
        while (s.equals(nextS) && rest.hasNext()) {
          nextT = rest.next();
          nextS = new Site(nextT.getPackage(), nextT.getClazz(), nextT.getLoc(), nextT.getLine(), nextT.getFile(),
              nextT.getMethodCallClass(), nextT.getMethodCallName());
        }
    ...
    public int hashCode() {
      ...
      result = prime * result + (threadsSeen == null ? 0 : threadsSeen.hashCode());
      return result;
    }
  ...
  static class FieldCoverageHandler implements ResultHandler<List<FieldCoverage>> {
