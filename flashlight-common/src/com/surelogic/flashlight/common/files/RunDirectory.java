...
public final class RunDirectory {
    ...
    static RunDirectory getFor(final File directory) {
        ...
        if (!directory.exists()) {
            throw new IllegalArgumentException(I18N.err(95,
                    directory.getAbsolutePath(), "it does not exist"));
        }
        if (!directory.isDirectory()) {
            throw new IllegalArgumentException(I18N.err(95,
                    directory.getAbsolutePath(), "it is not a directory"));
        }

        final File invalidFile = new File(directory,
                InstrumentationConstants.FL_INVALID_RUN);
        if (invalidFile.exists()) {
            return null;
        }

        final SourceZipFileHandles source = SourceZipFileHandles
                .getFor(directory);
        if (source == null) {
            return null;
        }
         ...
         * the machinery that existed to compute them.
         */
        final File headerFile = getFileFrom(directory,
                flashlightHeaderFileFilter, 152, 153);
        if (headerFile == null) {
         ...
         */
        final File[] rawDataFiles = getFilesFrom(directory,
        ...
        final List<RawDataFilePrefix> prefixInfos = new ArrayList<RawDataFilePrefix>(
                Arrays.asList(RawFileUtility.getPrefixesFor(rawDataFiles)));
        // Take out files if their prefix is not well formed
        for (final Iterator<RawDataFilePrefix> iter = prefixInfos.iterator(); iter
                .hasNext();) {
        ...
        final RawFileHandles rawFileHandles = RawFileUtility
                .getRawFileHandlesFor(directory, prefixInfos
                        .toArray(new RawDataFilePrefix[prefixInfos.size()]));
    ...
    private RunDirectory(@NonNull final RunDescription runDescription,
            @NonNull final File runDirHandle,
            @NonNull final SourceZipFileHandles sourceZipFileHandles,
    ...
    public String getHumanReadableSize() {
        return FileUtility.bytesToHumanReadableString(FileUtility
                .recursiveSizeInBytes(f_runDirHandle));
    }
    ...
    /**
     * Checks if this run has been, or is being, prepared by seeing if the
     * handle returned from {@link #getDatabaseDirectory()} exists.
     * 
    ...
    public String getHumanReadableDatabaseSize() {
        return FileUtility.bytesToHumanReadableString(FileUtility
                .recursiveSizeInBytes(getDatabaseDirectory()));
    }
    ...
    public HtmlHandles getHtmlHandles() {
        ...
        if (result == null) {
            throw new IllegalStateException("Unable to work on html within "
                    + f_runDirHandle.getAbsolutePath());
        } else {
            return result;
        }
    }
    ...
    /**
     * Gets an abstract representation of the list of queries containing no
     * data.
     * 
