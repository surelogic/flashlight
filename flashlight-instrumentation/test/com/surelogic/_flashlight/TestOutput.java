...
public class TestOutput extends TestCase {
  ...
  final CountDownLatch midGate = new CountDownLatch(THREADS);
  ...
  class ProgThread extends Thread {
    ...
    public void run() {
      ...
      /*
       * Output a few reads from a thread local object before the shared one
       * gets lots of events. Hopefully, these will get removed from the event
       * cache within the Refinery. If not you'll see a failure with up to
       * (THREADS * 2) extra events.
       */
      A o = new A();
