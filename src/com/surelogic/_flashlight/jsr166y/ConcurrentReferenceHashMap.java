...
package com.surelogic._flashlight.jsr166y;
 ...
 * expected concurrency for updates, configurable garbage collection semantics
 * of keys and values, and the optional use of reference-equality rather than
 * object-equality. This class obeys the same functional specification as
 * {@link java.util.Hashtable}, and includes versions of methods corresponding
 * to each method of <tt>Hashtable</tt>. However, even though all operations
 * are thread-safe, retrieval operations do <em>not</em> entail locking, and
 * there is <em>not</em> any support for locking the entire table in a way
 * that prevents all access. This class is fully interoperable with
 ...
 * 
 * <p>
 * This class supports strong, weak, and soft keys and values. By default keys
 ...
 * this table. In other words, in the default configuration, this table will not
 * prevent a key from being discarded by the garbage collector. Once a key has
 * been discarded by the collector, the corresponding entry is no longer visible
 * to this table; however, the entry may occupy space until a future table
 * operation decides to reclaim it. For this reason, summary functions such as
 * <tt>size</tt> and <tt>isEmpty</tt> might return a value greater than the
 * observed number of entries. In order to support a high level of concurrency,
 * stale entries are only reclaimed during blocking (usually mutating)
 * operations.
 * 
 * <p>
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V>
	...
	/**
	 * The default key reference type for this table, used when not otherwise
	 * specified in a constructor.
	 */
	static final ReferenceType DEFAULT_KEY_TYPE = ReferenceType.WEAK;

	/**
	 * The default value reference type for this table, used when not otherwise
	 * specified in a constructor.
	 */
	...
	/**
	 * Flags if reference-equality is used for this map rather than
	 * object-equality.
	 */
	...
	static final class HashEntry<K, V> {
		...
		HashEntry(K key, int hash, HashEntry<K, V> next, V value,
				...
				ReferenceType valueReferenceType, ReferenceQueue<K> refQueue) {
		...
		final V value(ReferenceType type) {
			return dereferenceValue(type, valueRef);
		}

		@SuppressWarnings("unchecked")
		final V dereferenceValue(ReferenceType type, Object value) {
			if (type != ReferenceType.STRONG && value instanceof Reference)
	...
	static final class Segment<K, V> extends ReentrantLock implements
		...
		Segment(int initialCapacity, float lf, ReferenceType keyReferenceType,
		...
		private boolean keyEq(Object src, Object dest) {
			return useReferenceEquality ? src == dest : src.equals(dest);
		}
		...
		HashEntry<K, V> newHashEntry(K key, int hash, HashEntry<K, V> next,
			...
			return new HashEntry<K, V>(key, hash, next, value,
					keyReferenceType, valueReferenceType, refQueue);
		}
		...
		V readValueUnderLock(HashEntry<K, V> e) {
			...
			try {
				removeStale();
				return e.value(valueReferenceType);
			} finally {
				unlock();
			}
		}
			...
			if (count != 0) { // read-volatile
				...
				while (e != null) {
					if (e.hash == hash && keyEq(key, e.key())) {
						...
						if (opaque != null)
							return e.dereferenceValue(valueReferenceType, opaque);
				...
				for (int i = 0; i < len; i++) {
					for (HashEntry<K, V> e = tab[i]; e != null; e = e.next) {
						...
						if (opaque == null)
							...
							v = e.dereferenceValue(valueReferenceType, opaque);
		...
		boolean replace(K key, int hash, V oldValue, V newValue) {
			...
			try {
				...
				if (e != null && oldValue.equals(e.value(valueReferenceType))) {
					replaced = true;
					e.setValue(newValue);
				}
				return replaced;
		...
		V replace(K key, int hash, V newValue) {
			...
			try {
				...
				V oldValue = null;
				if (e != null) {
					oldValue = e.value(valueReferenceType);
					e.setValue(newValue);
				}
				return oldValue;
			} finally {
				...
				if (e != null) {
					...
					if (next == null)
						...
						for (HashEntry<K, V> p = e; p != lastRun; p = p.next) {
							...
							newTable[k] = newHashEntry(key, p.hash, n, p
		...
		V remove(Object key, int hash, Object value, boolean weakRemove) {
			...
			try {
				...
				V oldValue = null;
				if (e != null) {
					V v = e.value(valueReferenceType);
					if (value == null || value.equals(v)) {
						...
						for (HashEntry<K, V> p = first; p != e; p = p.next) {
							...
							newFirst = newHashEntry(pKey, p.hash, newFirst, p
									.value(valueReferenceType));
						}
						tab[index] = newFirst;
						count = c; // write-volatile
	 ...
	 * types, load factor, concurrency level, and equality comparison scheme.
	 * 
	 ...
	 *            the reference type to use for values.
	 * @param useReferenceEquality
	 *            indicates that reference-equality should be used rather than
	 *            object-equality for this map. In other words, if <tt>true</tt>
	 *            then two keys <tt>k1</tt> and <tt>k2</tt> are considered
	 *            equal if and only if <tt>(k1==k2)</tt>. If <tt>false</tt>
	 *            then two keys <tt>k1</tt> and <tt>k2</tt> are considered
	 *            equal if and only if
	 *            <tt>(k1==null ? k2==null : k1.equals(k2))</tt>.
	 * @throws IllegalArgumentException
	 *             if the initial capacity is negative, or the load factor or
	 *             concurrencyLevel are nonpositive, or the key or value
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor,
		...
		for (int i = 0; i < this.segments.length; ++i)
			this.segments[i] = new Segment<K, V>(cap, loadFactor,
					...
					this.useReferenceEquality);
	}

	/**
	 * Creates a new, empty map with the specified initial capacity, load factor
	 * and concurrency level. The map uses the default reference types (weak
	 * keys, strong values) and object-equality comparisons.
	 * 
	 * @param initialCapacity
	 *            the initial capacity. The implementation performs internal
	 *            sizing to accommodate this many elements.
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor,
		...
		this(initialCapacity, loadFactor, concurrencyLevel, DEFAULT_KEY_TYPE,
				DEFAULT_VALUE_TYPE, DEFAULT_USE_REFERENCE_EQUALITY);
	}
	 ...
	 * concurrencyLevel (16), and object-equality comparisons.
	 * 
	 * @param initialCapacity
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor) {
		this(initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL,
				DEFAULT_KEY_TYPE, DEFAULT_VALUE_TYPE,
				DEFAULT_USE_REFERENCE_EQUALITY);
	}
	...
	/**
	 * Creates a new, empty map with the specified initial capacity, reference
	 * types and with default load factor (0.75), concurrencyLevel (16), and
	 ...
	 *            the reference type to use for values.
	 * @throws IllegalArgumentException
	 *             if the initial capacity of elements is negative or the key or
	 *             value reference type is null.
	 */
	public ConcurrentReferenceHashMap(int initialCapacity,
			ReferenceType keyReferenceType, ReferenceType valueReferenceType) {
		this(initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL,
				keyReferenceType, valueReferenceType,
				DEFAULT_USE_REFERENCE_EQUALITY);
	}

	/**
	 * Creates a new, empty map with the specified reference types and equality
	 * comparison scheme, and with default initial capacity, load factor (0.75),
	...
	/**
	 * Creates a new, empty map with the specified equality comparison scheme
	 * and with default initial capacity (16), reference types (weak keys,
	 ...
	 * types (weak keys, strong values), default load factor (0.75),
	 * concurrencyLevel (16), and object-equality comparisons.
	 */
	 ...
	 * map or 16 (whichever is greater), and a default load factor (0.75),
	 * concurrencyLevel (16), reference types (weak keys, strong values), and
	 * object-equality comparisons.
	 * 
	 * @param m
	 *            the map
	...
	/**
	 * Returns the type of references used for keys in this map.
	 * 
	 * @return the type of references used for keys in this map.
	 */
	...
	/**
	 * Returns the type of references used for values in this map.
	 * 
	 * @return the type of references used for values in this map.
	 */
	...
	/**
	 * Returns <tt>true</tt> if this map is using reference-equality rather
	 * than object-equality. In other words, if this method returns
	 * <tt>true</tt> then two keys <tt>k1</tt> and <tt>k2</tt> are
	 * considered equal if and only if <tt>(k1==k2)</tt>. If this method
	 * returns <tt>false</tt> then two keys <tt>k1</tt> and <tt>k2</tt>
	 * are considered equal if and only if
	 * <tt>(k1==null ? k2==null : k1.equals(k2))</tt>.
	 * 
	 * @return <tt>true</tt> if this map is using reference-equality, false
	 *         otherwise
	 */
	public boolean usingReferenceEquality() {
		return useReferenceEquality;
	}
	...
	final class ValueIterator extends HashIterator implements Iterator<V>,
			Enumeration<V> {
		public V next() {
			return super.nextEntry().value(valueReferenceType);
		}
	...
	final class EntryIterator extends HashIterator implements
		...
		public Map.Entry<K, V> next() {
			...
			return new WriteThroughEntry(e.key(), e.value(valueReferenceType));
		}
	}
			...
			try {
				...
				for (int i = 0; i < tab.length; ++i) {
					for (HashEntry<K, V> e = tab[i]; e != null; e = e.next) {
						...
						s.writeObject(key);
						s.writeObject(e.value(valueReferenceType));
					}
				}
			} finally {
				seg.unlock();
