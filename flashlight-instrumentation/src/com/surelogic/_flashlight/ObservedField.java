...
abstract class ObservedField {
  ...
  static final SingleThreadedField getSingleThreadedEventAbout(final long field, final ObjectPhantomReference receiver) {
  ...
  ObservedField(final ClassPhantomReference declaringType, final String fieldName, final int modifier) {
    /*
     * We create a lot of instances that we don't use to allow getInstance() to
     * run concurrently. It is possible, however unlikely, that we could wrap
     * f_observedFieldCount around.
     */
   ...
   * profiling will tell if this is a good approach (I hope it is and reflection
   * is suppose to be fast according to Ernst).
   */
   ...
   * @param rawQueue
   *          the queue to place a {@link FieldDefinition} event into if this
   *          field has not been observed previously.
   * @return the one {@link ObservedField} instance associated with the
   *         specified field.
   */
  static ObservedField getInstance(final String className, final String fieldName, final PostMortemStore.State state) {
  ...
  static ObservedField getInstance(final Field field, final PostMortemStore.State state) {
    return getInstance(field.getDeclaringClass(), field.getName(), state);
  }

  static ObservedField getInstance(final Class declaringType, final String fieldName, final PostMortemStore.State state) {
    ...
    final ObservedField result = Modifier.isStatic(mod) ? new Static(pDeclaringType, fieldName, mod)
        : new Instance(pDeclaringType, fieldName, mod);
   ...
   */
  private static Field getFieldInternal(final Class root, final String fname) throws NoSuchFieldException {
    try {
    ...
    throw new NoSuchFieldException(
        "Couldn't find field \"" + fname + "\" in class " + root.getCanonicalName() + " or any of its ancestors");
  }
  ...
  private static class Instance extends ObservedField {
    public Instance(final ClassPhantomReference declaringType, final String fieldName, final int modifier) {
  ...
  private static class Static extends ObservedField {
    public Static(final ClassPhantomReference declaringType, final String fieldName, final int modifier) {
      super(declaringType, fieldName, modifier);
    }
   ...
   */
  static class FieldInfo implements IFieldInfo {

    private final ConcurrentHashMap<Long, IdPhantomReference> long2pr = new ConcurrentHashMap<Long, IdPhantomReference>(50);

    public ConcurrentHashMap<Long, IdPhantomReference> asMap() {
      return long2pr;
    }
    ...
    @Override
    public void setLastThread(final long key, final IdPhantomReference thread) {
      final PhantomReference lastThread = long2pr.get(key);
      if (lastThread == SHARED_BY_THREADS) {
        return;
      }
      if (lastThread == null) {
        // First time to see this field: set to the current thread
        long2pr.put(key, thread);
        /*
        ...
        // Set field as shared
        long2pr.put(key, SHARED_BY_THREADS);
      }
    }
    ...
    public boolean getSingleThreadedFields(final SingleThreadedRefs refs) {
       ...
       * this.entrySet().iterator(); while (it.hasNext()) { Map.Entry<Long,
       * IdPhantomReference> e = it.next();
       */
      for (final Map.Entry<Long, IdPhantomReference> me : long2pr.entrySet()) {
        if (me.getValue() != SHARED_BY_THREADS) {
          added = true;
          refs.addField(ObservedField.getSingleThreadedEventAbout(me.getKey(), getReceiver()));
        }
      }
      return added;
    }
