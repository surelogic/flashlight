 ...
 * Generic enough to accommodate both method calls that are instrumented by
 * being placed in wrapper methods and method calls that are instrumented in
 * place (in the case of calls from interface initializers).
 */
public abstract class MethodCall {
    ...
    /**
     * @param opcode
     *            The opcode used to invoke the method.
     * @param owner
    ...
    public MethodCall(final RewriteMessenger msg,
        ...
        originalInsn = insn;
        returnType = Type.getReturnType(insn.desc);
    ...
    private final boolean testCalledMethodName(final String testOwner,
            final String testName) throws ClassNotFoundException {
        /*
         * Test the method name first: no sense fooling with the class model if
         * the method doesn't match.
         */
        return originalInsn.name.equals(testName)
                && classModel.getClass(testOwner).isAssignableFrom(
                        originalInsn.owner);
    }
     ...
     * 
     * @param arg
     *            A number greater than 0, where 1 indicates the first
     *            non-receiver argument, 2 the second non-receiver argument,
     *            etc.
     */
     ...
     * {@link #invokeMethod(MethodVisitor)} so that the labels for the
     * try-finally block can be inserted around the method invocation. This
     * keeps the block from being larger than necessary.
     */
    ...
    public final void instrumentMethodCall(
         ...
         * The other instrumentation calls must come before/after the
         * methodCall() call as appropriate.
         */
        ...
        mv.prependTryCatchBlock(beforeOriginalCall, afterOriginalCall,
                exceptionHandler, null);

        // Non-null if the call is interesting for happens-before events
        Result hbResult;
        try {
            hbResult = happensBefore.getHappensBefore(originalInsn.owner,
                    originalInsn.name, originalInsn.desc);
        } catch (final ClassNotFoundException e) {
            hbResult = null;
            messenger
                    .warning("Provided classpath is incomplete 7: couldn't find class "
                            + e.getMissingClass());
        }
        ...
        /*
         * stack is either ... or ..., nanoTime (long)
         */

        /* original method call */

        pushReceiverAndArguments(mv); // +X
        /*
         * stack is either ..., rcvr, arg1, ...., argN or ..., nanoTime (long),
         * rcvr, arg1, ..., argN
         */
        ...
        /*
         * stack is either ..., [returnValue] or ..., nanoTime (long),
         * [returnValue]
         */
        ...
        if (hbResult != null) {
            instrumentHappensBefore(mv, config, hbResult);
            /*
             * Stack is now definitely ..., [returnValue]
             */
        }
        ...
        /*
         * Exception handler: still want to report method exit when there is an
         * exception
         */
        ...
        /* Additional post-call instrumentation */
        instrumentAfterLockAndLockInterruptibly(mv, config, false); // +4 (+1
                                                                    // exception)
         ...
         * handler. If the method fails, there is no happens before
         * relationship.
         */
    ...
    // +4 Stack
    private void instrumentBeforeMethodCall(final MethodVisitor mv,
            final Configuration config) {
        if (config.instrumentBeforeCall) {
            // empty stack
            ByteCodeUtils.pushBooleanConstant(mv, true);
            // true
            pushReceiverForEvent(mv);
            // true, objRef
            pushSiteId(mv);
            // true, objRef, callSiteId (long)
            ByteCodeUtils.callStoreMethod(mv, config,
                    FlashlightNames.METHOD_CALL);
            // empty stack
        }
    }

    // +4 Stack
    private void instrumentAfterMethodCall(final MethodVisitor mv,
            final Configuration config) {
    ...
    // +4 Stack
    private void instrumentBeforeWait(final MethodVisitor mv,
            final boolean callerIsStatic, final Configuration config) {
        try {
            if (testCalledMethodName(FlashlightNames.JAVA_LANG_OBJECT,
                    FlashlightNames.WAIT) && config.instrumentBeforeWait) {
                // ...
                ByteCodeUtils.pushBooleanConstant(mv, true);
                // ..., true
                pushReceiverForEvent(mv);
            ...
            messenger
                    .warning("Provided classpath is incomplete 8: couldn't find class "
                            + e.getMissingClass());
        }
    }

    // +4 Stack
    private void instrumentAfterWait(final MethodVisitor mv,
            final boolean callerIsStatic, final Configuration config) {
        try {
            if (testCalledMethodName(FlashlightNames.JAVA_LANG_OBJECT,
                    FlashlightNames.WAIT) && config.instrumentAfterWait) {
                // ...
                ByteCodeUtils.pushBooleanConstant(mv, false);
                // ..., false
                pushReceiverForEvent(mv);
            ...
            messenger
                    .warning("Provided classpath is incomplete 9: couldn't find class "
                            + e.getMissingClass());
        }
    }

    // +3 Stack
    private void instrumentBeforeAnyJUCLockAcquisition(final MethodVisitor mv,
            final Configuration config) {
        try {
            if ((testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
                    FlashlightNames.LOCK)
                    || testCalledMethodName(
                            FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
                            FlashlightNames.LOCK_INTERRUPTIBLY) || testCalledMethodName(
                        FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
            ...
            messenger
                    .warning("Provided classpath is incomplete 10: couldn't find class "
                            + e.getMissingClass());
        }
    }

    // +4 Stack
    private void instrumentAfterLockAndLockInterruptibly(
            final MethodVisitor mv, final Configuration config,
            final boolean gotTheLock) {
        try {
            if ((testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
                    FlashlightNames.LOCK) || testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
            ...
            messenger
                    .warning("Provided classpath is incomplete 11: couldn't find class "
                            + e.getMissingClass());
        }
    }

    // +4 Stack
    private void instrumentAfterTryLockNormal(final MethodVisitor mv,
            final Configuration config) {
        try {
            if (testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
                    FlashlightNames.TRY_LOCK)
                    && config.instrumentAfterTryLock) {
                // ..., gotTheLock
            ...
            messenger
                    .warning("Provided classpath is incomplete 12: couldn't find class "
                            + e.getMissingClass());
        }
    }

    // +4 Stack
    private void instrumentAfterTryLockException(final MethodVisitor mv,
            final Configuration config) {
        try {
            if (testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
            ...
            messenger
                    .warning("Provided classpath is incomplete 13: couldn't find class "
                            + e.getMissingClass());
        }
    }
    ...
    private void instrumentAfterUnlock(final MethodVisitor mv,
            final Configuration config, final boolean releasedTheLock) {
        try {
            if (testCalledMethodName(
                    FlashlightNames.JAVA_UTIL_CONCURRENT_LOCKS_LOCK,
                    FlashlightNames.UNLOCK)
                    && config.instrumentAfterUnlock) {
                // ...
            ...
            messenger
                    .warning("Provided classpath is incomplete 14: couldn't find class "
                            + e.getMissingClass());
        }
    }

    private void instrumentHappensBefore(final MethodVisitor mv,
            final Configuration config, final Result result) {
        ...
        // ..., nanoTime (long), [return value]
        hb.invokeSwitch(new InstrumentationSwitch(this, mv, config,
                result.isExact, returnValueSize));
        // ..., [return value]
    ...
    private static final class InstrumentationSwitch implements
            HappensBeforeSwitch {
        ...
        private void swapNanoTimeAndReturnValue() {
            ...
            /*
             * returnValueSize == 0 means void return, so there isn't a return
             * value on the stack at all.
             */
        ...
        public void caseHappensBefore(final HappensBeforeRule hb) {
            ...
            // (long)
            /*
             * Push null if the call is exact, or the qualified type name if the
             * result is not exact.
             */
            pushTypeNameForDynamicTesting(hb);
            // ..., [return value], nanoTime (long), threadRef, id, callSideId
            // (long), [type name or null]
            ByteCodeUtils.pushBooleanConstant(mv, false); // Not a call-in
                                                          // situation
            // ..., [return value], nanoTime (long), threadRef, id, callSideId
            // (long), [type name or null], false
            ByteCodeUtils.callStoreMethod(mv, config,
        ...
        public void caseHappensBeforeCollection(
            ...
            if (hb.isParamReturnValue()) {
                if (returnValueSize == 2) {
                    /*
                     * this really shouldn't ever be the case because we expect
                     * the return value to be a object reference for our
                     * purposes. But let's generate legal JVM code for this case
                     * anyhow.
                     */
                    // ..., nanoTime (long), returnValue (long)
                    mv.visitInsn(Opcodes.DUP2_X2);
                    // ..., returnValue (long), nanoTime (long), returnValue
                    // (long)
                } else {
                    // ..., nanoTime (long), returnValue
                    mv.visitInsn(Opcodes.DUP_X2);
            ...
            pushTypeNameForDynamicTesting(hb);
            // ..., [return value], nanoTime (long), item, collection, id,
            // callSiteId (long), [type name or null]
            ByteCodeUtils.pushBooleanConstant(mv, false); // Not a call-in
                                                          // situation
            // ..., [return value], nanoTime (long), item, collection, id,
            // callSiteId (long), [type name or null], false
            ByteCodeUtils.callStoreMethod(mv, config,
