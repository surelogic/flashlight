...
public class HappensBeforeExecutor extends HappensBefore {

  public HappensBeforeExecutor(ClassHierarchy hbConfig) {
    super(hbConfig);
  }
  ...
  void parseRest(PreppedAttributes attributes, String id, long nanoStart, long nanoEnd, long inThread, long trace, long site)
    ...
    if (obj == ILLEGAL_ID) {
      SLLogger.getLogger().log(Level.SEVERE, "Missing obj in " + getXMLElementName());
      return;
    }
  ...
  private void insert(final String id, final long nanoTime, final long inThread, final long trace, final long obj,
  ...
  public void setup(final Connection c, final Timestamp start, final long startNS, final ScanRawFilePreScan scanResults)
    ...
    f_sourcePs = c.prepareStatement("INSERT INTO HAPPENSBEFORESOURCE (ID,OBJ,TS,INTHREAD,TRACE) VALUES (?,?,?,?,?)");
    f_targetPs = c.prepareStatement("INSERT INTO HAPPENSBEFORETARGET (ID,OBJ,TS,INTHREAD,TRACE) VALUES (?,?,?,?,?)");
  }
