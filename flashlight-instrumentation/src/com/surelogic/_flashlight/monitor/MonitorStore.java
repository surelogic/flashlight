...
public final class MonitorStore implements StoreListener {
  ...
  /**
   * Get the active analysis. This is the only thread-safe way for a console to
   ...
   */
  public MonitorStore() {
    f_lockNames = new ConcurrentReferenceHashMap<Long, String>(ReferenceType.STRONG, ReferenceType.STRONG,
        ConcurrentReferenceHashMap.STANDARD_HASH);

    f_rwLocks = new ConcurrentReferenceHashMap<Long, ReadWriteLockIds>(ReferenceType.STRONG, ReferenceType.STRONG,
        ConcurrentReferenceHashMap.STANDARD_HASH);
    ...
    tl_lockSet = new ThreadLocal<ThreadLocks>() {
      ...
      protected ThreadLocks initialValue() {
        ...
        final boolean isEDT = thread.getName().startsWith("AWT-EventQueue");
        final ThreadLocks ls = new ThreadLocks(thread.getName(), thread.getId(), isEDT, f_rwLocks);
        f_lockSets.add(ls);
        return ls;
      }
  ...
  public void init(final RunConf conf) {
    f_spec = new MonitorSpec(System.getProperty("com.surelogic.fieldSpec", ""), conf.getFieldDefs());
    f_conf = conf;
   ...
   *          access when {@code dcPhantom} is null} . If the {@code dcPhantom}
   *          is non- null} then this is null} .
   */
  @Override
  public void instanceFieldAccess(final boolean read, final Object receiver, final int fieldID, final long siteId,
    ...
    if (f_spec.isMonitoring(fieldID)) {
      tl_lockSet.get().field(fieldID, receiverId, rec.isUnderConstruction());
    }
   ...
   */
  @Override
  public void staticFieldAccess(final boolean read, final int fieldID, final long siteId, final ClassPhantomReference dcPhantom,
   ...
   */
  @Override
  public void arrayAccess(final boolean read, final Object receiver, final int index, final long siteId) {
    // Do nothing
  }
   ...
   *
   * @param before
   *          true} if the the class initializer is about to begin execution;
   *          false} if the class initializers has already completed execution.
   * @param class
   ...
   * constructor code that is executed. Therefore, explicit or implicit calls to
   * {@code super(..)} or {@code this(..)} can cause multiple blocks of code to
   * execute during object construction. The receiver object reported will, of
   * course, be the same for all pairs of constructor executions reported for
   * the construction of an object.
   *
   * @param before
   *          {@code true} indicates <i>before</i> the constructor execution,
   *          {@code false} indicates <i>after</i> the constructor execution.
   * @param receiver
   ...
   */
  @Override
  public void constructorExecution(final boolean before, final Object receiver, final long siteId) {
    // Do nothing
  }

  /**
   * Records that a method call occurred within the instrumented program.
   * Typically this is a call to a method from another method, however, the call
   * could originate in a constructor or an initializer.
   * <p>
   * This method also dispatches this event properly if the method call is to an
   * <i>interesting</i> method with regard to the program's concurrency.
   ...
   * @param receiver
   *          the object instance the method is being called on, or {@code null}
   ...
   */
  @Override
  public void methodCall(final boolean before, final Object receiver, final long siteId) {
    ...
    if (receiver instanceof ReadWriteLock) {
      ...
      if (!f_knownRWLocks.addReadWriteLock(p)) {
        ...
        final ReadWriteLockIds ids = new ReadWriteLockIds(id, read, write);
  ...
  /**
   * Records that the instrumented program is attempting to acquire an intrinsic
   ...
   */
  @Override
  public void beforeIntrinsicLockAcquisition(final Object lockObject, final boolean lockIsThis, final long siteId) {
    // Do nothing
  }
   ...
   */
  @Override
  public void afterIntrinsicLockAcquisition(final Object lockObject, final boolean lockIsThis, final long siteId) {
    final long lockId = Phantom.of(lockObject).getId();
    if (!f_lockNames.containsKey(lockId)) {
      if (lockObject instanceof Class) {
        f_lockNames.put(lockId, "class " + ((Class) lockObject).getName() + '-' + lockId);
      }
      f_lockNames.put(lockId, lockObject.getClass().getName() + '-' + lockId);
    }
   ...
   * </ul>
   * See the Java Language Specification (3rd edition) section 17.8 <i>Wait Sets
   * and Notification</i> for the semantics of waiting on an intrinsic lock. An
   * intrinsic lock is a {@code synchronized} block or method.
   *
   * @param before
   *          {@code true} indicates <i>before</i> the method call,
   *          {@code false} indicates <i>after</i> the method call.
   * @param lockObject
   *          the object being waited on (i.e., the thread should be holding a
   *          lock on this object).
   * @param withinClass
   *          the class where the event occurred, may be {@code null}.
   * @param line
   ...
   */
  @Override
  public void intrinsicLockWait(final boolean before, final Object lockObject, final boolean lockIsThis, final long siteId) {
    // Do nothing
  }

  /**
   * Records that the program has released an intrinsic lock. An intrinsic lock
   * is a {@code synchronized} block or method.
   *
   * @param lockObject
   *          the object being synchronized (i.e., the lock).
   * @param withinClass
   ...
   */
  @Override
  public void afterIntrinsicLockRelease(final Object lockObject, final boolean lockIsThis, final long siteId) {
   ...
   */
  @Override
  public void beforeUtilConcurrentLockAcquisitionAttempt(final Object lockObject, final long siteId) {
    // Do nothing
  }
   ...
   *          obtained, {@code false} indicates the attempt failed and the lock
   *          was not obtained (due to an exception or a false return from a
   *          {@link Lock#tryLock()} or
   ...
   */
  @Override
  public void afterUtilConcurrentLockAcquisitionAttempt(final boolean gotTheLock, final Object lockObject, final long siteId) {
    if (lockObject instanceof Lock) {
      ...
      f_conf.logAProblem(String.format(fmt, gotTheLock ? "holding" : "failed-to-acquire", lockObject, siteId));
      return;
    }
  }
   ...
   *          {@code true} indicates the attempt succeeded and the lock was
   *          released, {@code false} indicates the attempt failed and the lock
   *          was not released (due to an exception that was likely caused
   *          because the thread was not holding the lock).
   * @param lockObject
   *          the {@link Lock} object in use.
   * @param withinClass
   *          the class where the event occurred, may be {@code null}.
   ...
   */
  @Override
  public void afterUtilConcurrentLockReleaseAttempt(final boolean releasedTheLock, final Object lockObject, final long siteId) {
    if (lockObject instanceof Lock) {
      if (releasedTheLock) {
        final ObjectPhantomReference lockPhantom = Phantom.ofObject(lockObject);
      ...
      f_conf.logAProblem(String.format(fmt, releasedTheLock ? "released" : "failed-to-release", lockObject, siteId));
      return;
    }
  }

  @Override
  public void instanceFieldInit(final Object receiver, final int fieldId, final Object value) {
    // Do nothing
  }
  ...
  @Override
  public void garbageCollect(final List<? extends IdPhantomReference> references) {
  ...
  class AlertsCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return ALERTS + " - display any and all alerts that have been triggered.";
    }
  ...
  class LockSetsCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return LOCKSETS + " - display known lock set information for all observed fields";
    }
  ...
  class RaceConditionsCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return RACECONDITIONS + " - display all detected potential race conditions";
    }
  ...
  class SharedCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return SHARED + " - display all observed fields that are shared between threads";
    }
  ...
  class PropsCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return PROPS + " - List all configurable monitor properties, and their current values.";
    }
    ...
    public String handle(final String command) {
      if (PROPS.equalsIgnoreCase(command)) {
        ...
        return String.format("%s=%s\n%s=%s\n%s=%s\n%s=%s\n", FIELD_SPEC, f_spec.getFieldSpec(), EDT_FIELDS, aSpec.getEDTSpec(),
            LOCKSET_FIELDS, aSpec.getLockSetSpec(), SHARED_FIELDS, aSpec.getSharedSpec());
      }
      return null;
    }
  ...
  class SetCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return SET + "- Set one of the following properties: "
          + Arrays.toString(new String[] { FIELD_SPEC, EDT_FIELDS, SHARED_FIELDS, LOCKSET_FIELDS });
    }
    ...
    public String handle(final String command) {
      ...
      if (m.matches()) {
        ...
        if (FIELD_SPEC.equalsIgnoreCase(prop)) {
          ...
          reviseAlerts(new AlertSpec(val, null, null, f_conf.getFieldDefs()));
          return String.format("Monitoring fields matching %s for Swing policy violations.", val);

        } else if (SHARED_FIELDS.equalsIgnoreCase(prop)) {
          reviseAlerts(new AlertSpec(null, val, null, f_conf.getFieldDefs()));
          return String.format("Ensuring fields matching %s are not shared.", val);

        } else if (LOCKSET_FIELDS.equalsIgnoreCase(prop)) {
          reviseAlerts(new AlertSpec(null, null, val, f_conf.getFieldDefs()));
          return String.format("Ensuring fields matching %s always have a lock set.", val);
        } else {
  ...
  class DescribeCommand implements ConsoleCommand {
    ...
    @Override
    public String getDescription() {
      return DESCRIBE + " - describe all observed fields in the programming matching the given regular expression";
    }
  ...
  public Collection<? extends ConsoleCommand> getCommands() {
    return Arrays
        .asList(new ConsoleCommand[] { new ListCommand(), new AlertsCommand(), new DeadlocksCommand(), new LockSetsCommand(),
            new RaceConditionsCommand(), new SharedCommand(), new SetCommand(), new PropsCommand(), new DescribeCommand() });
  }

  @Override
  public void happensBeforeThread(String id, Thread callee, long siteId, String typeNa, long nanoTime) {
    // Do nothing
  }

  @Override
  public void happensBeforeObject(String id, Object object, long siteId, String typeName, long nanoTime) {
    // Do nothing
  }

  @Override
  public void happensBeforeCollection(String id, Object collection, Object item, long siteId, String typeName, long nanoTime) {
    // Do nothing
  }

  @Override
  public void happensBeforeExecutor(String id, Object object, long siteId, String typeName, long nanoTime) {
    // Do nothing
  }
