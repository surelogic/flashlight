...
public class FieldAssignment extends RangedEvent {
  ...
  public void parse(final PreppedAttributes attributes) throws SQLException {
    ...
    final Long receiver = attributes.containsKey(AttributeType.RECEIVER) ? attributes.getLong(AttributeType.RECEIVER) : null;
    if (receiver == null) {
      return;
    }
    if (field == ILLEGAL_FIELD_ID || receiver == ILLEGAL_RECEIVER_ID) {
      SLLogger.getLogger().log(Level.SEVERE, "Missing field or receiver in field-assignment");
    }
    if (receiver == ILLEGAL_RECEIVER_ID || receiver < f_begin || receiver > f_end) {
      return;
    }
    if (!f_scanResults.couldBeReferencedObject(receiver) || f_scanResults.isSynthetic(field)) {
      skipped++;
      return;
    }
    inserted++;
  ...
  private void insert(final long field, final long value, final Long receiver) throws SQLException {
    int idx = 1;
