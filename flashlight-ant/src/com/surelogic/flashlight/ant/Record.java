...
public class Record extends Task {
  ...
  public void execute() throws BuildException {
    ...
    if (runFolder.exists()) {
      throw new BuildException("Run directory with name " + datedName + " already exists.");
    } else {
      runFolder.mkdir();
    }
  ...
  private Path instrumentFiles(final File runFolder) {
    ...
    final File projectFolder = new File(runFolder, InstrumentationConstants.FL_PROJECTS_FOLDER_LOC);
    if (!projectFolder.mkdirs()) {
      throw new BuildException("Could not create project directory.");
    }
    final File sourceFolder = new File(runFolder, InstrumentationConstants.FL_SOURCE_FOLDER_LOC);
    ...
    for (final Inspect p : inspects) {
      ...
      if (l.isDirectory()) {
        ...
        if (!dest.mkdir()) {
          throw new BuildException("Could not create directory " + dest);
        }
    ...
    final File externalFolder = new File(runFolder, InstrumentationConstants.FL_EXTERNAL_FOLDER_LOC);
  ...
  private void runProgram(final String name, final File runFolder, final Date now, final Path instrumentedFiles) {
    if (bootclasspath != null) {
    ...
    if (jarfile != null) {
      for (final Inspect p : inspects) {
        if (p.getLoc().equals(jarfile)) {
          j.setJar(new File(new File(runFolder, InstrumentationConstants.FL_PROJECTS_FOLDER_LOC), p.getLoc().getName())
              .getAbsolutePath());
        }
      }
      if (libraries != null) {
        for (final String e : libraries.list()) {
          if (e.equals(jarfile)) {
            j.setJar(new File(new File(runFolder, InstrumentationConstants.FL_EXTERNAL_FOLDER_LOC), new File(e).getName())
                .getAbsolutePath());
          }
        }
      }
    }
    addVMArg(InstrumentationConstants.FL_FIELDS_FILE,
        new File(runFolder, InstrumentationConstants.FL_FIELDS_FILE_LOC).getAbsolutePath());
    addVMArg(InstrumentationConstants.FL_CLASS_HIERARCHY_FILE,
        new File(runFolder, InstrumentationConstants.FL_CLASS_HIERARCHY_FILE_LOC).getAbsolutePath());
    addVMArg(InstrumentationConstants.FL_SITES_FILE,
        new File(runFolder, InstrumentationConstants.FL_SITES_FILE_LOC).getAbsolutePath());
    ...
    addVMArg(InstrumentationConstants.FL_RAWQ_SIZE, Integer.toString(InstrumentationConstants.FL_RAWQ_SIZE_DEFAULT));
    addVMArg(InstrumentationConstants.FL_REFINERY_SIZE, Integer.toString(InstrumentationConstants.FL_REFINERY_SIZE_DEFAULT));
    addVMArg(InstrumentationConstants.FL_OUTQ_SIZE, Integer.toString(InstrumentationConstants.FL_OUTQ_SIZE_DEFAULT));
    addVMArg(InstrumentationConstants.FL_CONSOLE_PORT, Integer.toString(InstrumentationConstants.FL_CONSOLE_PORT_DEFAULT));
    addVMArg(InstrumentationConstants.FL_OUTPUT_TYPE, InstrumentationConstants.FL_OUTPUT_TYPE_DEFAULT.toString());
    addVMArg(InstrumentationConstants.FL_COLLECTION_TYPE, InstrumentationConstants.FL_COLLECTION_TYPE_DEFAULT.toString());
