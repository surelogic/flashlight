...
public final class PromptToPrepAllRawData extends SLUIJob {
  ...
  private static final PerspectiveAdapter PA = new PerspectiveAdapter() {
    ...
    public void perspectiveActivated(final IWorkbenchPage page, final IPerspectiveDescriptor perspective) {
      if (FlashlightPerspective.class.getName().equals(perspective.getId())) {
        createAndSchedule();
      }
    }
  };
  ...
  public IStatus runInUIThread(final IProgressMonitor monitor) {
     ...
     * avoid double prompting the user to prepare data. It may not work in all
     * cases but should eliminate most of them.
     * 
     * In particular if the dialog is already up and the user exits another
     * instrumented program then the data model will update and trigger another
     * instance of this job to run. Without this check the user would get two
     * prompts to prep the data.
     */
    final boolean onlyPromptToPrepAllRawDataJobRunning = EclipseUtility.getActiveJobCountOfType(PromptToPrepAllRawData.class) == 1;
    SLLogger.getLogger().fine(
        "[PromptToPrepAllRawData] onlyPromptToPrepAllRawDataJobRunning = " + onlyPromptToPrepAllRawDataJobRunning);
    if (!onlyPromptToPrepAllRawDataJobRunning) {
      return Status.OK_STATUS; // bail
    }

    final Set<RunDirectory> notPrepped = RunManager.getInstance().getNotPreparedRunDirectories();
