...
public class DefinitionEventGenerator {
  ...
  DefinitionEventGenerator(final RunConf conf, final BlockingQueue<List<Event>> outQueue) {
    f_conf = conf;
    f_outQueue = outQueue;
  ...
  private Map<String, List<ClassInfo>> loadClassInfo(SitesReader sitesReader) {
    ...
    try {
      ...
      if (name == null) {
        ...
        f_conf.log("Could not locate com.surelogic._flashlight.SitesConf, trying " + name + '.');
    ...
    /*
     * Create ClassInfo objects for the remaining site-less classes. These are
     * classes that are not instrumented, but possibly referenced by
    ...
    for (final Map.Entry<String, List<FieldInfo>> entry : fieldsMap.entrySet()) {
      ...
      final FieldInfo[] fields = finfo == null ? NO_FIELDS : finfo.toArray(NO_FIELDS);
      final ClassInfo info = new ClassInfo("<unknown>", classname, NO_SITES, fields);
  ...
  class ClassVisitor extends IdPhantomReferenceVisitor {
    ...
    void visit(final ClassPhantomReference r) {
      ...
      List<Event> events = outputClassDefs(r.getName(), r.getId(), new ArrayList<Event>());
      PostMortemStore.putInQueue(f_outQueue, events);
    }
  }

  List<Event> outputClassDefs(final String name, final long id, final List<Event> events) {
    final List<ClassInfo> infos = classDefs.remove(name);
    if (infos == null) {
      /*
       * if (name.startsWith("java")) { return 0; } if
       * ("com.surelogic._flashlight.ObservedField$FieldInfo" .equals(name)) {
  ...
  List<Event> outputFieldDefs(final long id, final List<Event> events, final ClassInfo info) {
  ...
  static class ClassInfo extends AbstractList<ClassInfo> {
    ...
    ClassInfo(final String file, final String clazz, final SiteInfo[] sites, final FieldInfo[] fields) {
      fileName = file;
      className = clazz;
      this.sites = sites;
      this.fields = fields;
  ...
  static class SiteInfo {
    ...
    SiteInfo(final long id, final String name, final String desc, final int mod, final int line, String methodName,
        String methodClass, String methodDesc, int methodModifier, boolean inInterface) {
      this.id = id;
      memberName = name;
      memberDesc = desc;
      memberModifier = mod;
    ...
    void accept(final long declaringType, final List<Event> events, final ClassInfo info) {
      events.add(new StaticCallLocation(id, memberName, memberDesc, memberModifier, line, info.fileName, declaringType,
          isInInterface, methodClass, methodName, methodDesc, methodModifier));
    }
  }
  ...
  static class StringTable extends HashMap<String, String> {
    private static final long serialVersionUID = -2616477230273622964L;
