...
public class ReadWriteLock extends Event {
  ...
  public void parse(final PreppedAttributes attributes) throws SQLException {
    ...
    if ((id == ILLEGAL_ID) || (readLock == ILLEGAL_ID) || (writeLock == ILLEGAL_ID)) {
      SLLogger.getLogger().log(Level.SEVERE, "Missing id, read-lock-id, or write-lock-id in read-write-lock-definition");
      return;
    }
    insert(id, readLock, writeLock);
  }

  private void insert(final long id, final long readLock, final long writeLock) throws SQLException {
    int idx = 1;
    f_ps.setLong(idx++, id);
