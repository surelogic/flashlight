...
public final class StaticCallLocation extends AbstractPrep {
  ...
  public static final LongSet validSites = new LongScatterSet();
  ...
  public void parse(final PreppedAttributes attributes) throws SQLException {
    ...
    f_ps.setString(idx++, getMethodCode(loc, attributes.getBoolean(IN_INTERFACE), attributes.getInt(LOCATIONMOD)));
    ...
    f_ps.setString(idx++, callName == null ? null : getMethodCode(callName, false, attributes.getInt(METHODCALLMOD)));
    if (doInsert) {
      f_ps.addBatch();
      if (++count == 10000) {
        f_ps.executeBatch();
  ...
  private static String getMethodCode(String name, boolean isInInterface, int mod) {
    ...
    if (isInInterface && !(Modifier.isStatic(mod) || Modifier.isAbstract(mod))) {
      code.append("D");
    }
  ...
  public void setup(final Connection c, final Timestamp start, final long startNS, final ScanRawFilePreScan scanResults)
    ...
    f_ps = c.prepareStatement(
        "INSERT INTO SITE (Id,AtLine,InClass,InFile,Location,LocationSpec,LocationCode,MethodClass,MethodCall,MethodSpec,MethodCode) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
  }
