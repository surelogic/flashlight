...
public final class QueryResultsView extends AbstractQueryResultsView {
  ...
  private void parseAndRunQuery(final String queryUrl) {
    try {
      final AdHocQueryFullyBound query = getManager().parseQueryUrl(queryUrl);
      EclipseQueryUtility.scheduleQuery(query, getManager().getDataSource().getCurrentAccessKeys());
    } catch (IllegalStateException problem) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(213, queryUrl), problem);
    }
  }

  private void parseAndJumpToLine(final RunDirectory run, final String url) {
    try {
      Map<String, String> params = SimpleHTMLPrinter.extractParametersFromURL(url);
      params.put(AdHocManager.DATABASE, run.getRunIdString());
