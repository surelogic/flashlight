...
public class PostMortemRefinery extends Thread {
  ...
  public PostMortemRefinery(PostMortemStore store, RunConf conf, DefinitionEventGenerator defs,
      BlockingQueue<List<? extends IdPhantomReference>> gcQueue, BlockingQueue<List<Event>> outQueue, int refinerySize) {
    super("flashlight-refinery");
    f_store = store;
    f_conf = conf;
    f_defs = defs;
    f_shutdown = false;
    f_toRegister = new ArrayBlockingQueue<PostMortemRefinery.StateReference>(StoreConfiguration.getThreadRegistrationQueueSize());
  ...
  public void run() {
    ...
    for (;;) {
      ...
      boolean timesUp;
      if (f_lastRollover == 0) {
        timesUp = curTime - f_refineryStart > f_conf.getFileEventInitialDuration();
      } else {
        timesUp = curTime - f_lastRollover > f_conf.getFileEventDuration();
      }
      boolean isCheckpoint = count > f_conf.getFileEventCount() || timesUp;
      if (isCheckpoint) {
        /*
         * place all events preceding the checkpoint time into the event cache
         * before the checkpoint event
         */
        List<Event> buf = new ArrayList<Event>();
        for (Iterator<StateReference> iter = activeThreads.iterator(); iter.hasNext();) {
    ...
    f_conf.log("refinery completed (" + f_garbageCollectedObjectCount + " object(s) garbage collected : " + f_threadLocalFieldCount
        + " thread-local fields observed)");
  }

  /**
   * Examines each garbage collected object and cleans up our information about
   * shared fields and thread-local fields.
   *
   * @param filter
   */
  ...
  /**
   * When the refinery finishes up any remaining fields that have been observed
   * to be thread-local can have events about them removed. This method also
   * cleans up the collections used to determine if a field is thread-local or
   * shared.
   */
  ...
  /**
   * Remove all mappings within the {@link #f_fieldToThread} map that are within
   * the garbage collected object.
   *
   * @param pr
   *          the phantom of the object.
   */
  private void removeThreadLocalFieldsWithin(final List<Event> events, final SingleThreadedRefs refs, final PhantomReference pr) {
    /*
  ...
  private void markAsSingleThreaded(final List<Event> events, final SingleThreadedRefs refs) {
  ...
  private boolean transferEventsToOutQueue(final boolean flush) {
    int transferCount = flush ? f_eventCache.size() : f_eventCache.size() - f_eventCacheSize;
    if (transferCount > 0) {
      ...
      PostMortemStore.putInQueue(f_outQueue, new ArrayList<Event>(toTransfer));
      toTransfer.clear();
      return true;
    } else {
      return false;
   ...
   * collected. The local queue is kept on the reference explicitly in order to
   * allow any final events in the thread to be cleared.
   *
   * @author nathan
   *
   */
  private static class StateReference extends WeakReference<PostMortemStore.State> {

    final BlockingQueue<Event> localQueue;

    public StateReference(com.surelogic._flashlight.PostMortemStore.State referent) {
      super(referent);
