...
public final class RunControlDialog extends Dialog implements
        IRunManagerObserver, TickListener {
    ...
    static final Runnable f_openDialogJob = new Runnable() {
        ...
        public void run() {
            if (f_dialogInstance == null) {
                ...
                final TimingSource ts = new SWTTimingSource(1,
                        TimeUnit.SECONDS, shell.getDisplay());
    ...
    RunControlDialog(Shell parentShell,
            @NonNull final TimingSource disposeOnClose) {
        super(parentShell);
        /*
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        final ScrolledComposite sc = new ScrolledComposite(composite,
                SWT.V_SCROLL);
     ...
     * returned. Be sure to look at the superclass implementation when making
     * any changes.
     * 
     * @param parent
     *            the parent composite to contain the button bar.
     * @return the button bar control. The returned control's layout data must
     *         be an instance of <code>GridData</code>.
     */
    @Override
    protected Control createButtonBar(Composite parent) {
        ...
        if ((search.getStyle() & SWT.ICON_CANCEL) == 0) {
            ...
            layout.numColumns++;
            toolBar.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false,
                    false));
        }
    ...
    void showSearchPrompt(final Text search) {
        ...
        search.setForeground(search.getDisplay().getSystemColor(
                SWT.COLOR_DARK_GRAY));
    }
    ...
    final class RunControlItem {
        ...
        RunControlItem(@NonNull final Composite parent,
                @NonNull final LaunchedRun lrun) {
            if (parent == null) {
            ...
            f_image.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false,
                    false));
            ...
            f_runLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
                    false));
            f_runLabel.setFont(JFaceResources.getFontRegistry().getBold(
                    JFaceResources.HEADER_FONT));
            f_stateLabel = new Label(f_labels, SWT.NONE);
            f_stateLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
                    false));
        ...
        String getTimeInformation(boolean finished) {
            if (!finished) {
                ...
                return "Started at "
                        + SLUtility.toStringNoDayHMS(launchDate)
                        + " running for "
                        + SLUtility.toStringDurationS(durationMS,
        ...
        final Image f_javaRunning = SLImages.scaleImage(
                SLImages.getImage(CommonImages.IMG_JAVA_APP, false), f_size);
        final Image f_javaFinished = SLImages.getGrayscaleImage(f_javaRunning);
        final Image f_androidRunning = SLImages.scaleImage(
                SLImages.getImage(CommonImages.IMG_ANDROID_APP, false), f_size);
        final Image f_androidFinished = SLImages
                .getGrayscaleImage(f_androidRunning);
        ...
        void updateGUIInformation() {
            ...
            f_runLabel.setForeground(finished ? f_runLabel.getDisplay()
            ...
            if (f_lrun.isFinishedCollectingData()) {
                ...
                if (tracker != null) {
                    ...
                    if (f_bottom instanceof ProgressBar) {
                        ...
                        prepProgressBar.setLayoutData(new GridData(SWT.FILL,
                    ...
                    /*
                     * If the progress bar is already tracking this prepare job
                     * we do not want to resetup the callbacks. We set the
                     * tracker as the progress bar's data and check if they are
                     * the same.
                     */
                    ...
                    if (tracker != data) {
                        ...
                        final int percentage = fixUpPercentage(tracker
                                .getPercentageOrState());
                ...
                if (f_bottom instanceof Label) {
                    ...
                    durationLabel.setLayoutData(new GridData(SWT.FILL,
                            SWT.CENTER, true, false));
                    durationLabel.setForeground(EclipseColorUtility
                            .getSubtleTextColor());
        ...
        final void buttonPressed() {
            if (f_lrun.isFinishedCollectingData() || f_lrun.isCancelled()) {
                RunManager.getInstance().setDisplayToUser(
                        f_lrun.getRunIdString(), false);
            } else {
                RunManager.getInstance().requestDataCollectionToStop(
                        f_lrun.getRunIdString());
            }
        }
    ...
    void updateGUIModel() {
        EclipseUIUtility.nowOrAsyncExec(new Runnable() {
            ...
            public void run() {
                ...
                final ArrayList<LaunchedRun> launchedRuns = RunManager
                        .getInstance().getLaunchedRuns();

                for (Iterator<LaunchedRun> iterator = launchedRuns.iterator(); iterator
                        .hasNext();) {
                ...
                for (int i = 0; i < launchedRuns.size(); i++) {
                    ...
                    if (i < f_guiModel.size()) {
                        ...
                        final RunControlItem gui = new RunControlItem(
                                f_dialogArea, lrun);
                        f_guiModel.add(gui);
                    }
                }
