...
public abstract class Lock extends Event {
  ...
  public void parse(final PreppedAttributes attributes) throws SQLException {
    ...
    if (nanoTime == ILLEGAL_ID || inThread == ILLEGAL_ID || trace == ILLEGAL_ID || lock == ILLEGAL_ID) {
      SLLogger.getLogger().log(Level.SEVERE, "Missing nano-time, thread, site, or lock in " + getXMLElementName());
      return;
    }
    final Timestamp time = getTimestamp(nanoTime);
    LockId lockNode = new LockId(lock, getType());
    final long id = f_rowInserter.insertLock(false, time, inThread, trace, lockNode, object, getState(), success, lockIsThis);
    f_rowInserter.event(id, time, inThread, trace, lockNode, object, getState(), success != Boolean.FALSE);
  }

  /**
   * If the lock event is capable of failing, returns <code>true</code> when it
   * succeeds and <code>false</code> when it fails. If the lock event always
   * succeeds regardless this method should return null.
   * 
   * @param attr
   * @return
   */
  ...
  @Override
  public final void setup(final Connection c, final Timestamp start, final long startNS, final ScanRawFilePreScan scanResults)
      throws SQLException {
