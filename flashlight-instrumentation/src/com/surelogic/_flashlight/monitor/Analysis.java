...
final class Analysis extends Thread {
  ...
  public synchronized void gcReferences(final List<? extends IdPhantomReference> references) {
  ...
  public synchronized AlertInfo getAlerts() {
    ...
    final Set<FieldDef> shared = new HashSet<FieldDef>(sharedFieldViolations);
  ...
  public synchronized DeadlockInfo getDeadlocks() {
    return new DeadlockInfo(master.getLockOrders(), master.getDeadlocks(), new HashMap<Long, String>(store.getLockNames()));
  }
  ...
  public synchronized SharedFieldInfo getShared() {
    return new SharedFieldInfo(conf.getFieldDefs(), shared.calculateSharedFields(), shared.calculateUnsharedFields());
  }

  @Override
  public synchronized String toString() {
    return new FullInfo(getAlerts(), master.getLockSetInfo(), getShared(), getDeadlocks()).toString();
  }
