...
public final class LockCycleGraph extends AbstractQueryResultCustomDisplay {
  ...
  private void drawNode(final GC gc, final Node node) {
    ...
    gc.drawText(node.getLabel(), nr.x + PAD, nr.y + PAD, SWT.DRAW_TRANSPARENT);
  }
  ...
  private void drawEdge(final GC gc, final Node from, final Node to) {
    ...
    final Point ctrl = getControlPointFor(cFrom, cTo, 35.0);
  ...
  private Point getExitPointFor(@NonNull GC gc, @NonNull Node node, Point outside) {
    final Rectangle nr = getCenteredRectangleFor(gc, node);
    final Point c = getCenterOf(node);
  ...
  private static Point getMidPointBetween(Point p1, Point p2) {
    ...
    return new Point(p2.x + SLUtility.safeLongToInt(Math.round(hx)), p2.y + SLUtility.safeLongToInt(Math.round(hy)));
  }
  ...
  private static Point getControlPointFor(Point p1, Point p2, double distance) {
    int dx = p2.x - p1.x;
    int dy = p2.y - p1.y;
    ...
    int y = getYOnLine(tslope, yInt, x);
