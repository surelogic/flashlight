...
public final class FlashlightDBConnection extends DerbyConnection {
  ...
  private static final ConcurrentHashMap<String, FlashlightDBConnection> INSTANCES = new ConcurrentHashMap<>();
  ...
  /**
   * Create a new connection to a database at the given location. If the folder
   * specified does not exist, it will be created when the database is booted.
   * 
   * @param dbLoc
   *          a valid path name
   */
   ...
   * 
   * @param dbLoc
   *          a file pointing to a non-existent location or a derby database.
   */
  ...
  public Connection getConnection() throws SQLException {
    ...
    try {
      ...
      if ("XJ040".equals(e.getSQLState()) && "XSDB6".equals(((SQLException) e.getCause().getCause()).getSQLState())) {
  ...
  public Connection transactionConnection() throws SQLException {
    // We want to use table level locking where appropriate in Flashlight,
    // for performance reasons.
    Connection conn = super.transactionConnection();
