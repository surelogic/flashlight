...
public final class Prep extends Task {
	...
	public void execute() throws BuildException {
		...
		final ConsoleJob consoleJob = new ConsoleJob(
				new AntSLProgressMonitor.Factory(this));

		final DBConnection dbConnection = FlashlightDBConnection
				.getInstance(destdir);
		try {
			dbConnection.bootAndCheckSchema();
		} catch (final Exception e) {
			throw new BuildException(
					"Failed to initialized database connection", e);
		}
		...
		if (status.getSeverity() != SLSeverity.OK
				|| status.getCode() != SLStatus.OK) {
			throw new BuildException("Flashlight data preparation failed: "
	...
	public static void main(final String[] args) throws Exception {
		...
		final ConsoleJob consoleJob = new ConsoleJob(
				PrintWriterSLProgressMonitor.getFactory(pw));
		...
		if (status.getSeverity() != SLSeverity.OK
				|| status.getCode() != SLStatus.OK) {
			System.out.println("Flashlight data preparation failed: "
					+ status.getMessage());
