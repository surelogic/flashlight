...
public final class Phantom {
  ...
  public static ObjectPhantomReference ofObject(final Object o, final long id) {
    ...
    if (o instanceof Thread) {
      ...
      throw new IllegalArgumentException("the object cannot be an instance of Class");
    } else {
  ...
  /**
   * Removes all available elements from the queue of garbage collected objects
   * and adds them, as {@link IdPhantomReference}s, into the given collection.
  ...
  public static int drainTo(final Collection<IdPhantomReference> c) {
    int count = 0;
    while (true) {
      final IdPhantomReference pr = (IdPhantomReference) f_collected.poll();
      if (pr == null) {
        return count;
      }
      c.add(pr);
