...
public class LockSetInfo {
   ...
   * @param statics
   * @param lockSetFields
   *          a map of all lock sets for fields currently being observed in the
   *          program, keyed by field first and then receiver
   * @param noLockSetFields
   * @param instances
   */
  LockSetInfo(final FieldDefs defs, final Map<Long, Set<Long>> statics, final Set<Long> lockSetFields,
      final Set<Long> noLockSetFields, final Map<Long, Map<Long, Set<Long>>> instances) {
  ...
  /**
   * Whether or not the given field has always been observed to have a lock set.
   * 
   * @param field
   * @return whether the field always has a lock set, or <code>true</code> if it
    ...
    if (field.isStatic()) {
      ...
      if (map != null) {
        for (final Entry<Long, Set<Long>> receivers : map.entrySet()) {
          b.append(String.format("\t%s - %s\n", receivers.getKey(), receivers.getValue()));
        }
      }
    }
    return b.toString();
  ...
  String fieldInfo(final FieldDef field) {
    return String.format("%s - %s\n", field.getQualifiedFieldName(), field.isFinal() ? "final" : "");
  }
