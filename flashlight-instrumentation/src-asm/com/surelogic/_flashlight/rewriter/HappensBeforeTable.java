...
final class HappensBeforeTable {
  ...
  /*
   * First check the method name. This is a map from String (method name) to a
   * list of HappensBefore records. Only contains those records where the callIn
   * attribute if false.
   */
  final Map<String, List<Record>> methodMap = new HashMap<String, List<Record>>();
   ...
   * attribute if true.
   */
  final Map<String, List<Record>> callInMethodMap = new HashMap<String, List<Record>>();

  final LaunchType launchType;

  public HappensBeforeTable(final LaunchType type, final HappensBeforeConfig config, final ClassAndFieldModel m,
  ...
  private <T extends HappensBeforeRule> void add(final Map<String, List<T>> map, final RewriteMessenger messenger) {
    for (final Map.Entry<String, List<T>> e : map.entrySet()) {
      for (final HappensBeforeRule hb : e.getValue()) {
        final Map<String, List<Record>> mm = hb.isCallIn() ? callInMethodMap : methodMap;
        ...
        try {
          ...
          /*
           * Couldn't find the class that contains the happens-before method, so
           * we output a warning, and skip it.
           */
          messenger.warning("Provided classpath is incomplete 6: couldn't find class " + ex.getMissingClass()
   ...
   *         scenario that method participates in. If the call does not invoke a
   *         happens-before method, the set is empty. The <code>isExact</code>
   *         attribute of a result object is <code>true</code> if the method
   *         definitely calls a happens-before method. If <cod>false</code>, the
   *         dynamic type of the receiver needs to be checked to see if it is
   *         assignable to the class type named in the {@link HappensBeforeRule}
   *         object referenced by <code>hb</code>.
   * @throws ClassNotFoundException
   */
  public Set<Result> getHappensBefore(final String internalClassName, final String methodName, final String methodDesc)
      throws ClassNotFoundException {
  ...
  /**
   * Test if the given method description overrides a callIn method. Meant to be
   * called while a method is being instrumented to see if the happens-before
   * Store calls should be made.
   * 
   * @param internalClassName
   ...
   * @return A set of HappensBeforRule objects, one for each happens-before
   *         call-in scenario the method participates in.
   * @throws ClassNotFoundException
   */
  public Set<HappensBeforeRule> isInsideHappensBefore(final String internalClassName, final String methodName,
  ...
  private final class Record {
    ...
    Record(final HappensBeforeRule hb) throws ClassNotFoundException {
      this.hb = hb;
  ...
  public static final class Result {
    ...
    Result(final boolean isExact, final HappensBeforeRule hb) {
