...
public final class SiteIdFactory {
  ...
  long nextId = 0L;
  final SortedMap<SiteInfo, Long> sitesToIds = new TreeMap<SiteInfo, Long>();
  final SortedMap<Long, SiteInfo> idsToSites = new TreeMap<Long, SiteInfo>();
  String sourceFileName;
  String className;
  boolean isInterface;
  String callingMethodName;
  String callingMethodDesc;
  int callingMethodAccess;

  private static class SiteInfo implements Comparable<SiteInfo> {
    final int lineOfCode;
    final String calledMethodName;
    final String calledMethodOwner;
    final String calledMethodDesc;

    public SiteInfo(final int lineOfCode, final String calledMethodName, final String calledMethodOwner,
        final String calledMethodDesc) {
      super();
    ...
    SiteInfo(final int lineOfCode, final SiteInfo siteInfo) {
      this(lineOfCode, siteInfo.calledMethodName, siteInfo.calledMethodOwner, siteInfo.calledMethodDesc);
    }
    ...
    public int hashCode() {
      final int prime = 31;
      int result = 17;
      result = prime * result + (calledMethodDesc == null ? 0 : calledMethodDesc.hashCode());
      result = prime * result + (calledMethodName == null ? 0 : calledMethodName.hashCode());
      result = prime * result + (calledMethodOwner == null ? 0 : calledMethodOwner.hashCode());
    ...
    public int compareTo(final SiteInfo o) {
      int cmp = lineOfCode < o.lineOfCode ? -1 : lineOfCode == o.lineOfCode ? 0 : 1;
      if (cmp == 0) {
  ...
  public void setMethodLocation(final String sourceFileName, final String className, final boolean isInterface,
      final String callingMethodName, final String callingMethodDesc, final int access) {
  ...
  public long getSiteId(final int lineNumber, final String calledMethodName, final String calledMethodOwner,
      final String calledMethodDesc) {
    SiteInfo site = new SiteInfo(lineNumber, calledMethodName, calledMethodOwner, calledMethodDesc);
  ...
  private void recordSiteToDatabase(final ClassAndFieldModel model, final long siteId, final SiteInfo site)
    ...
    if (site.calledMethodName == null) {
      ...
      final int access = c.getMethodAccess(site.calledMethodName, site.calledMethodDesc);
      pw.println(access);
    }
  }
  ...
  public void closeMethod(final ClassAndFieldModel model) throws ClassNotFoundException {
    /*
     * If possible, reassociate the site associated with line number -1 with the
     * first non-negative line number holding a site.
     */
    ...
    if (emptySiteId != null && firstRealLineNumber != null) {
      idsToSites.put(emptySiteId, new SiteInfo(firstRealLineNumber, emptySite));
    }
