...
public class PostMortemStore implements StoreListener {
  ...
  public static final class State {
    ...
    State() {
      localQueue = new ArrayBlockingQueue<Event>(StoreConfiguration.getRawQueueSize());
  ...
  public PostMortemStore() {
    ...
    f_gcQueue = new ArrayBlockingQueue<List<? extends IdPhantomReference>>(GC_DRIFT);
  ...
  public void init(final RunConf conf) {
    ...
    if (StoreConfiguration.hasOutputPort()) {
      ...
      outputStrategy = new CheckpointingOutputStreamStrategy(f_conf, factory, outType);
    }

    // Initialize Queues

    putInQueue(f_outQueue, startingEvents);
    IdPhantomReference.addObserver(new IdPhantomReferenceCreationObserver() {
      @Override
      public void notify(final ClassPhantomReference o, final IdPhantomReference r) {
        /*
         * Create an event to define this object.
         */
        putInQueue(tl_withinStore.get(), new ObjectDefinition(o, r));
      }
    });
    DefinitionEventGenerator defs = new DefinitionEventGenerator(conf, f_outQueue);
    // Start Refinery and Depository
    final int refinerySize = StoreConfiguration.getRefinerySize();
    f_refinery = new PostMortemRefinery(this, f_conf, defs, f_gcQueue, f_outQueue, refinerySize);
    f_refinery.start();
    f_depository = new Depository(f_conf, f_outQueue, outputStrategy);
    f_depository.start();
    f_conf.log("collection started (rawQ=" + StoreConfiguration.getRawQueueSize() + " : refinery=" + refinerySize + " : outQ="
  ...
  public void methodCall(final boolean before, final Object receiver, final long siteId) {
    ...
    if (receiver instanceof ReadWriteLock) {
      ...
      if (f_rwLocks.addReadWriteLock(p)) {
        ...
        final Event e = new ReadWriteLockDefinition(p, Phantom.ofObject(rwl.readLock()), Phantom.ofObject(rwl.writeLock()));
        putInQueue(state, e);
      }
    }
  ...
  public void beforeIntrinsicLockAcquisition(final Object lockObject, final boolean lockIsThis, final long siteId) {
    State state = tl_withinStore.get();
    final Event e = new BeforeIntrinsicLockAcquisition(lockObject, lockIsThis, siteId, state);
    putInQueue(state, e);
  }

  @Override
  public void afterIntrinsicLockAcquisition(final Object lockObject, boolean lockIsThis, final long siteId) {
    State state = tl_withinStore.get();
    final Event e = new AfterIntrinsicLockAcquisition(lockObject, siteId, state, lockIsThis);
    putInQueue(state, e);
  }

  @Override
  public void intrinsicLockWait(final boolean before, final Object lockObject, boolean lockIsThis, final long siteId) {
    final Event e;
    State state = tl_withinStore.get();
    if (before) {
      e = new BeforeIntrinsicLockWait(lockObject, siteId, state, lockIsThis);
    } else {
      e = new AfterIntrinsicLockWait(lockObject, siteId, state, lockIsThis);
    }
    putInQueue(state, e);
  ...
  @Override
  public void afterIntrinsicLockRelease(final Object lockObject, boolean lockIsThis, final long siteId) {
    State state = tl_withinStore.get();
    final Event e = new AfterIntrinsicLockRelease(lockObject, siteId, state, lockIsThis);
    putInQueue(state, e);
  }
  ...
  public void beforeUtilConcurrentLockAcquisitionAttempt(final Object lockObject, final long siteId) {
    if (lockObject instanceof Lock) {
      ...
      final Event e = new BeforeUtilConcurrentLockAcquisitionAttempt(ucLock, siteId, state);
      putInQueue(state, e);
    } else {
  ...
  public void afterUtilConcurrentLockAcquisitionAttempt(final boolean gotTheLock, final Object lockObject, final long siteId) {
    if (lockObject instanceof Lock) {
      State state = tl_withinStore.get();
      final Lock ucLock = (Lock) lockObject;
      final Event e = new AfterUtilConcurrentLockAcquisitionAttempt(gotTheLock, ucLock, siteId, state);
      putInQueue(state, e);
    } else {
  ...
  public void afterUtilConcurrentLockReleaseAttempt(final boolean releasedTheLock, final Object lockObject, final long siteId) {
    if (lockObject instanceof Lock) {
      State state = tl_withinStore.get();
      final Lock ucLock = (Lock) lockObject;
      final Event e = new AfterUtilConcurrentLockReleaseAttempt(releasedTheLock, ucLock, siteId, state);
      putInQueue(state, e);
    } else {
  ...
  public void instanceFieldInit(final Object receiver, final int fieldId, final Object value) {
    putInQueue(tl_withinStore.get(), new FieldAssignment(receiver, fieldId, value));
  }
  ...
  /**
   * Puts an event into a blocking queue. This operation will block if the queue
   * is full and it will ignore any interruptions.
   *
   * @param queue
  ...
  static <T> void putInQueue(final BlockingQueue<T> queue, final T e) {
    ...
    while (!done) {
      try {
        ...
        /*
         * We are within a program thread, so another program thread interrupted
         * us. We ensure the event still gets put into the raw queue, then
         * appropriately set the interrupted status.
         */
        interrupted = true;
      }
    }
  ...
  /**
   * Used by the refinery to notify the store of when a read/write lock has been
   * garbage collected.
   *
   * @param pr
   */
  ...
  @Override
  public void happensBeforeThread(String id, Thread callee, long siteId, String typeName, long nanoTime) {
    State state = tl_withinStore.get();
    putInQueue(state, new HappensBeforeThread(id, Phantom.ofThread(callee), siteId, state, nanoTime));
  }

  @Override
  public void happensBeforeObject(String id, Object object, long siteId, String typeName, long nanoTime) {
    State state = tl_withinStore.get();
    putInQueue(state, new HappensBeforeObject(id, Phantom.ofObject(object), siteId, state, nanoTime));
  }

  @Override
  public void happensBeforeCollection(String id, Object collection, Object item, long siteId, String typeName, long nanoTime) {
    State state = tl_withinStore.get();
    putInQueue(state, new HappensBeforeCollection(id, Phantom.ofObject(collection), item == null ? null : Phantom.ofObject(item),
  ...
  public void happensBeforeExecutor(String id, Object object, long siteId, String typeName, long nanoTime) {
    State state = tl_withinStore.get();
    putInQueue(state, new HappensBeforeExecutor(id, Phantom.ofObject(object), siteId, state, nanoTime));
  }
