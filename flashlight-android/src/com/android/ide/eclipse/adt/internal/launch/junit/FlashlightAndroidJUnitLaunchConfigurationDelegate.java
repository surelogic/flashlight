...
public class FlashlightAndroidJUnitLaunchConfigurationDelegate extends FlashlightAndroidLaunchConfigurationDelegate {
  ...
  protected FLData doBuild(RunId runId, final ILaunchConfiguration launchConfig, final IProject project,
    ...
    return AndroidBuildUtil.doFullIncrementalDebugBuildForTest(runId, launchConfig, project, monitor);
  }
  ...
  protected String doLaunch(final ILaunchConfiguration configuration, final String mode, final IProgressMonitor monitor,
    ...
    if (targetAppPackage == null) {
      AdtPlugin.displayError(LaunchMessages.LaunchDialogTitle, String.format(LaunchMessages.AndroidJUnitDelegate_NoTargetMsg_3s,
    ...
    final IAndroidLaunchAction junitLaunch = new FlashlightAndroidJUnitLaunchAction(junitLaunchInfo);

    Job job = new Job("Junit Launch") { //$NON-NLS-1$
      @Override
      protected IStatus run(IProgressMonitor m) {
        controller.launch(project, mode, applicationPackage, testAppPackage, targetAppPackage, manifestData.getDebuggable(),
            manifestData.getMinSdkVersionString(), junitLaunch, config, androidLaunch, monitor);
  ...
  /**
   * Get the target Android application's package for the given instrumentation
   * runner, or <code>null</code> if it could not be found.
   *
   ...
   *
   * @param configuration
   *          the {@link ILaunchConfiguration} to retrieve the test package info
   *          from
  ...
  private String getTestPackage(ILaunchConfiguration configuration) {
    ...
    String containerHandle = getStringLaunchAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_CONTAINER, configuration);
   ...
   *
   * @param configuration
   *          the {@link ILaunchConfiguration} to retrieve the test class info
   *          from
   ...
   *
   * @param configuration
   *          the {@link ILaunchConfiguration} to retrieve the test method info
   *          from
  ...
  private TestSize getTestSize(ILaunchConfiguration configuration) {
    String testSizeAnnotation = getStringLaunchAttribute(AndroidJUnitLaunchConfigDelegate.ATTR_TEST_SIZE, configuration);
   ...
   * <p/>
   * If a runner is stored in the given <code>configuration</code>, will return
   * that. Otherwise, will try to find the first valid runner for the project.
   * If a runner can still not be found, will return <code>null</code>, and will
   ...
   * @param configuration
   *          the {@link ILaunchConfiguration} to retrieve the attribute from
  ...
  static void setJUnitDefaults(ILaunchConfigurationWorkingCopy config) {
    ...
    config.setAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_RUNNER_KIND, TestKindRegistry.JUNIT3_TEST_KIND_ID);
  }
}
