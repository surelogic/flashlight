...
public class SocketOutputStrategy extends EventVisitor {
  ...
  final Factory f_fact;
  private final Thread f_connectThread;
  final OutputType f_outType;
  volatile boolean connecting;

  public SocketOutputStrategy(final RunConf conf, final Factory factory, final OutputType outType) {
    f_conf = conf;
    f_fact = factory;
    f_outType = outType;
    f_conf
        .log("Depository will connect to output port " + StoreConfiguration.getOutputPort() + " with output type " + outType + ".");
    connecting = true;
  ...
  class ConnectToHost extends Thread {
    ...
    public void run() {
      try {
        ...
        try {
          ...
          // time to shut down.
          if (f_socket != null) {
            OutputStream stream = OutputType.getOutputStreamFor(f_socket.getOutputStream(), f_outType);
          ...
          f_conf.logAProblem("Could not connect to socket and create output stream, shutting down instrumentation now..", e);
        }
        if (!success) {
          Store.shutdown();
        }
    ...
    void connectToHost() {
      try {
        ...
        f_conf.log(String.format("Listening for connections on %d at %s.", port, new Date().toString()));
        ...
        f_conf.logAProblem("Could not connect to socket and create output stream.", e);
      }
    }
