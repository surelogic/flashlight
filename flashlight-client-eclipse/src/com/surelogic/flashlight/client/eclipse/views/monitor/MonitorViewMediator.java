...
public class MonitorViewMediator {
  ...
  MonitorStatus f_monitorStatus;

  MonitorViewMediator(final Composite status, final Label statusImage, final Label runText, final Text fieldsSelector,
      final Button fieldsButton, final Tree fieldsTree, final Tree locksTree, final Text edtSelector, final Button edtButton,
      final Tree edtTree, final TextViewer listing) {
    f_status = status;
  ...
  private class SpecListener implements SelectionListener, ModifyListener {
    ...
    SpecListener(final String specType, final Text widget, final Button button) {
      f_specType = specType;
      f_widget = widget;
      f_button = button;
    }
    ...
    public void widgetSelected(final SelectionEvent e) {
      final Job job = EclipseUtility.toEclipseJob(
          new SendCommandToFlashlightMonitorJob(f_monitorStatus, String.format("set %s=%s", f_specType, f_widget.getText())));
      job.setSystem(true);
      job.schedule();
  ...
  public void init() {
    SpecListener l = new SpecListener(FIELD_SPEC, f_fieldsSelector, f_fieldsButton);
  ...
  private void searching(final MonitorStatus status) {
    ...
    f_runText.setText(status.getRunId());
    f_status.layout();
