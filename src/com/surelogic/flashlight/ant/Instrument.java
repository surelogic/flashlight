...
public final class Instrument extends Task {
	...
	public static final class Jar implements InstrumentationSubTask {
		...
		public void add(final RewriteManager manager) {
			...
			if (unjar) {
				...
				manager.addJarToJar(srcfile, new File(destdir, srcfile
						.getName()), runtimeJar);
			}
		}
	}
	...
	public void addConfiguredDirs(final Directories dirs) {
		...
		Instrument.this
				.log(
						MessageFormat
								.format(
										"Expanding list of directories using list=\"{0}\", delimeters=\"{1}\", source pattern=\"{2}\", destination directory pattern=\"{3}\", destination jar file pattern=\"{4}\", replacement pattern=\"{5}\", runtime jar file=\"{6}\"",
										list, delimeters, srcDirPattern,
										destDirPattern, destFilePattern,
										replace, runtime), Project.MSG_VERBOSE);
		...
		final StringTokenizer st = new StringTokenizer(dirs.getList(), dirs
				.getDelimeters());
		while (st.hasMoreTokens()) {
			...
			if (destDirPattern != null) {
				...
				subTasks
						.add(new Directory(new File(srcdir), new File(destdir)));
			} else {
	...
	private void processJars(final File srcdir, final File destdir,
		...
		for (final File child : srcdir.listFiles()) {
			if (recurse && child.isDirectory()) {
				final File newDest = unjar ? destdir : new File(destdir, child
						.getName());
	...
	private void checkParameters() throws BuildException {
		...
		if (extdirs == null) {
			...
			log(
					"Getting the Java extension directories from the runtime system",
					Project.MSG_VERBOSE);
