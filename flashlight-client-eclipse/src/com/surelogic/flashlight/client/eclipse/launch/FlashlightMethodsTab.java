...
public final class FlashlightMethodsTab extends AbstractLaunchConfigurationTab {
  Button useDefaultMethods;

  ListViewer extraFilesViewer;
  Button addButton;
  Button removeButton;
  ...
  public void initializeFrom(final ILaunchConfiguration config) {
    ...
    try {
      useDefault = config.getAttribute(FlashlightPreferencesUtility.USE_DEFAULT_INDIRECT_ACCESS_METHODS, useDefault);
    } catch (final CoreException e) {
      // Eclipse CommonTab is silent here, so are we
    }

    try {
      xtraFiles = config.getAttribute(FlashlightPreferencesUtility.ADDITIONAL_INDIRECT_ACCESS_METHODS, xtraFiles);
    } catch (final CoreException e) {
      // Eclipse CommonTab is silent here, so are we
    }
  ...
  public void performApply(final ILaunchConfigurationWorkingCopy config) {
    config.setAttribute(FlashlightPreferencesUtility.USE_DEFAULT_INDIRECT_ACCESS_METHODS, useDefaultMethods.getSelection());
    config.setAttribute(FlashlightPreferencesUtility.ADDITIONAL_INDIRECT_ACCESS_METHODS, extraFiles);
  }
  ...
  public void setDefaults(final ILaunchConfigurationWorkingCopy config) {
    config.setAttribute(FlashlightPreferencesUtility.USE_DEFAULT_INDIRECT_ACCESS_METHODS, true);
    config.setAttribute(FlashlightPreferencesUtility.ADDITIONAL_INDIRECT_ACCESS_METHODS, Collections.<String> emptyList());
  }
}
