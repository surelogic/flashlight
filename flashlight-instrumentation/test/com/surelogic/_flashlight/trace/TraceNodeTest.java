...
public class TraceNodeTest extends TestCase {
    ...
    /**
     * Ensures that we don't have a memory leak in or representation when
     * tracing the same trace paths over and over.
     * 
     * @throws InterruptedException
     */
    public void testTraceReuse() throws InterruptedException {
        final int depth = 8, branching = 9, threads = 8;
        ...
        assertEquals(48427561, root.getNodeCount());
        guessMemUsage(root, depth, branching);
        System.out.printf("Elapsed trace reuse time: %dms",
    ...
    static void guessMemUsage(TraceNode node, int depth, int branching) {
        ...
        System.out
                .printf("%dM Used for a tree of depth %d and branching factor %d (%d nodes)\n",
                        runtime.totalMemory() / MEGABYTE, depth, branching,
                        node.getNodeCount());
    }
