 ...
 */
class FlashlightInstrumentationRunnerValidator {
     ...
     *
     * @param javaProject
     *            the {@link IJavaProject} for the Android project to validate
     */
    FlashlightInstrumentationRunnerValidator(IJavaProject javaProject) {
        mJavaProject = javaProject;
        ManifestData manifestData = AndroidManifestHelper
                .parseForData(javaProject.getProject());
        init(manifestData);
    }
     ...
     *
     * @param project
     *            the {@link IProject} for the Android project to validate
     * @throws CoreException
     *             if a fatal error occurred in initialization
     */
    FlashlightInstrumentationRunnerValidator(IProject project) throws CoreException {
        this(BaseProjectHelper.getJavaProject(project));
    }

    /**
     * Initializes the InstrumentationRunnerValidator with an existing
     * {@link AndroidManifestHelper}
     *
     * @param javaProject
     *            the {@link IJavaProject} for the Android project to validate
     * @param manifestData
     *            the {@link ManifestData} for the Android project
     */
    FlashlightInstrumentationRunnerValidator(IJavaProject javaProject,
    ...
    /**
     * Helper method to determine if given manifest has a
     * <code>SdkConstants.LIBRARY_TEST_RUNNER
     * </code> library reference
     *
     * @param manifestParser
     *            the {@link ManifestData} to search
     ...
     * runner.
     *
     * @return fully qualified instrumentation class name. <code>null</code> if
     *         no valid instrumentation can be found.
     */
    @Nullable
    ...
    /**
     * Helper method to determine if specified instrumentation can be used as a
     * test runner
     *
     * @param instrumentation
     *            the instrumentation class name to validate. Assumes this
     *            instrumentation is one of {@link #getInstrumentationNames()}
     * @return <code>INSTRUMENTATION_OK</code> if valid, otherwise returns error
     *         message
     */
    String validateInstrumentationRunner(String instrumentation) {
        ...
        if (!instrumentation.equals(SdkConstants.CLASS_INSTRUMENTATION_RUNNER)) {
            // Ideally, we'd check if the class extends instrumentation test
            // runner.
            // However, the Google Instrumentation Test Runner extends Google
            // Instrumentation, and not a test runner,
            // so we just check that the super class is Instrumentation.
            String result = BaseProjectHelper.testClassForManifest(
                    mJavaProject, instrumentation,
